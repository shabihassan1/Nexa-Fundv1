...
exports.rewardTierController = void 0;
const database_1 = require("../config/database");
/**
 * Reward Tier Controller
 * Handles all reward tier-related operations
 */
exports.rewardTierController = {
    /**
     * Get all reward tiers for a campaign
     */
    getRewardTiersByCampaign: async (req, res) => {
        try {
            const { campaignId } = req.params;
            const rewardTiers = await database_1.prisma.rewardTier.findMany({
                where: { campaignId },
                include: {
                    _count: {
                        select: {
     ...
     */
    getRewardTierById: async (req, res) => {
        try {
            const { id } = req.params;
            const rewardTier = await database_1.prisma.rewardTier.findUnique({
                where: { id },
                include: {
                    campaign: {
                        select: {
     ...
     */
    createRewardTier: async (req, res) => {
        try {
            ...
            // Create reward tier
            const rewardTier = await database_1.prisma.rewardTier.create({
                data: {
                    campaignId,
                    title,
                    description,
     ...
     */
    updateRewardTier: async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (minimumAmount)
                updateData.minimumAmount = parseFloat(minimumAmount);
            // Update reward tier
            const updatedRewardTier = await database_1.prisma.rewardTier.update({
                where: { id },
                data: updateData,
                include: {
                    _count: {
     ...
     */
    deleteRewardTier: async (req, res) => {
        try {
            const { id } = req.params;
            // Get user from auth middleware
            const userId = req.user?.id;
            ...
            // Delete reward tier
            await database_1.prisma.rewardTier.delete({
                where: { id }
            });
...
//# sourceMappingURL=rewardTier.controller.js.map